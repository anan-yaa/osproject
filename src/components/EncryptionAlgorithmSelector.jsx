...
ciphertext = public_key.encrypt(
    ...
    padding.OAEP(
      ...
      imageUrl: 'https://media.geeksforgeeks.org/wp-content/uploads/20200518124317/RSA3.png' // You'll add your RSA diagram image URL here
    },
    ecc: {
      name: 'ECC',
...
public_pem = public_key.public_bytes(
    ...
    format=serialization.PublicFormat.SubjectPublicKeyInfo
      ...
      imageUrl: 'https://www.vmware.com/media/blt8c9a8aaca0ffd4ac/bltf43dda427c8639e5/66d186618950de33cfc38385/elliptic-curve-cryptography-diagram.png' // You'll add your ECC diagram image URL here
    },
    aes: {
      name: 'AES',
...
def encrypt_aes_gcm(plaintext, key):
    ...
    return (iv, ciphertext, encryptor.tag)`,
      imageUrl: 'https://media.geeksforgeeks.org/wp-content/uploads/20240628164747/aes.png' // You'll add your AES diagram image URL here
    },
    chacha20: {
      name: 'ChaCha20',
...
def decrypt_chacha20(ciphertext, key, nonce):
    ...
    return decryptor.update(ciphertext) + decryptor.finalize()`,
      imageUrl: 'https://xilinx.github.io/Vitis_Libraries/security/2019.2/_images/chhacha20_detail.png' // You'll add your ChaCha20 diagram image URL here
    }
  };
  ...
  // SVG diagrams for encryption algorithms
  const AlgorithmDiagram = ({ algorithm }) => {
    if (!algorithm || !algorithms[algorithm].imageUrl) {
      return (
    ...
    return (
      <div className="p-2 bg-white rounded-lg shadow-md border border-indigo-200 overflow-hidden">
        <img 
          src={algorithms[algorithm].imageUrl} 
          alt={`${algorithms[algorithm].name} diagram`} 
      ...
      <div className="w-full h-full">
        <div className="w-full bg-white/90 backdrop-blur-md shadow-2xl overflow-hidden">
          <div className="w-full bg-gradient-to-r from-violet-600 to-fuchsia-600 p-6 text-white text-center">
            <div>
              <CodepenIcon className="inline-block mr-2 text-white-300" />
              <h1 className="text-3xl font-extrabold text-white inline-block">Implementing Security Algorithms</h1>
              <CodepenIcon className="inline-block ml-2 text-white-300" />
              <p className="text-white/80 mt-2">Master the art of modern encryption</p>
            </div>
          </div>
          
          <div className="p-6">
            <h2 className="text-xl font-bold mb-4 text-center text-indigo-800">Choose your encryption algorithm to learn more about it</h2>
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-xl p-6 shadow-lg border-2 border-indigo-400">
                ...
                <div className="mb-6">
                  <h3 className="text-lg font-semibold mb-3 flex items-center text-indigo-800">
                    ...
                    <span>Algorithm Diagram</span>
                  </h3>
                  <div className="bg-gradient-to-r from-gray-50 to-indigo-50 p-4 rounded-lg border border-indigo-200">
                    {algorithms[selectedAlgorithm].imageUrl ? (
                      <img 
                        src={algorithms[selectedAlgorithm].imageUrl} 
                        alt={`${algorithms[selectedAlgorithm].name} diagram`} 
                        className="w-full h-auto object-contain max-h-64"
                      />
                    ) : (
                      <div className="flex flex-col items-center justify-center p-8 text-indigo-400">
                        <p className="mb-3">No diagram available</p>
                        <p className="text-sm text-center">Add an image URL to the <code className="bg-indigo-100 px-1 py-0.5 rounded text-indigo-600">imageUrl</code> property for {algorithms[selectedAlgorithm].name}</p>
                      </div>
                    )}
                  </div>
                </div>
